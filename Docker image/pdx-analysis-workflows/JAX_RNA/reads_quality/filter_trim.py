...
2.  Statistics file: The pipeline uses several criteria from this
...
class FastqRead(object):
    ...
    def __init__(self, fastqs, odir=None, suffix='_filtered_trimmed'):
        ...
        self.hq_reads = 0
        self.output_reads = 0
        self.min_trimmed_length = sys.maxint # use maxint if you are using python < 3.0 else maxsize
...
def parse_args():
    parser = argparse.ArgumentParser(description="Perform filtering and trimming of paired end fastq files",
                                     usage='%(prog)s [options]')
    parser.add_argument("-p", "--hq_pct", default="70", help="Percentage of bases that must be high quality [70]")
    parser.add_argument('-f', '--filter_hq', default="30", help='Numeric quality value to pass filtering [30]')
    parser.add_argument('-t', '--trim_hq', default="30", help='Numeric quality value to not be trimmed [30]')
    parser.add_argument('-m', '--min_len_pct', default="70", help='Minimum read length after trimming to '
                                                                  'retain read. (percentage 0-100) [70]')
    parser.add_argument('-M', '--min_pct_hq_reads', default=0, help='Minimum percentage of reads classified as High '
                                                                    'Quality reads (percentage 0-100) [0]')
    parser.add_argument('-j', '--sample_name', default="single", help='Sample name')
    parser.add_argument('-11', '--forward_sample_output', default="", help='Name for forward sample output')
    parser.add_argument('-12', '--reverse_sample_output', default="", help='Name for reverse sample output')
    parser.add_argument('-ssf', '--summary_stat_file', default="", help='Name and path for summary stats file')
    parser.add_argument('-5', '--trim_5', action="store_true", help="Trim 5' end as well as 3' [False]")
    parser.add_argument('-s', '--suffix', default='_filtered_trimmed', help='Suffix to construct the output file name '
                                                                            '[_filtered_trimmed]')
    parser.add_argument('-S', '--single_end', action="store_true", help="Single end with multiple fastq files [False]")
    parser.add_argument('-d', '--directory', dest='odir', default='.',
                        help='Directory in which to write the output files [current directory]')
    parser.add_argument('-i', '--timestamp', action='store_true', help='Emit a timestamp ever 1,000,000 reads [False]')
...
def main():
    ...
    src1 = args.odir + os.path.split(args.fastqs[::2][0])[1] + '_filtered_trimmed'
    # dest1 = args.fastqs[::2][1]

    if paired_end:
        src2 = args.odir + os.path.split(args.fastqs[1::2][0])[1] + '_filtered_trimmed'
        # dest2 = args.fastqs[1::2][1]
        print("[INFO] "+src1 + " " + src2)
    else:
        print("[INFO] "+src1)

    try:
        shutil.move(src1, args.forward_sample_output)
        if paired_end:
            shutil.move(src2, args.reverse_sample_output)
            shutil.move(
                args.odir + os.path.split(args.fastqs[::2][0])[1] + "_" + os.path.split(args.fastqs[1::2][0])[1] + "_stat",
                args.summary_stat_file)
        else:
            shutil.move(args.odir + os.path.split(args.fastqs[::2][0])[1] + "_stat", args.summary_stat_file)
    except Exception as e:
        print('Error saving results to the file -> %s' % e)
        sys.exit(1)
